...
class DesktopTranscriptUploader {
public:
    DesktopTranscriptUploader(AgentConfig config, Logger& logger, IAsrProvider& asr_provider);
    DesktopTranscriptUploader(
        AgentConfig config,
        Logger& logger,
...
private:
    void flush_unlocked();
    void process_pcm_chunk(std::vector<std::byte> pcm, const AudioFormat& format);
    void send_transcript(const AsrResult& asr, int duration_ms, double rms, const std::string& microphone_id);
    std::size_t target_chunk_bytes(const AudioFormat& format) const;

    AgentConfig config_;
    Logger& logger_;
    IAsrProvider& asr_provider_;
