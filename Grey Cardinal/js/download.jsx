...
const PLATFORMS = [
  { id:'win', icon:'windows', name:'Windows', sub:'Grey Cardinal Daemon for Windows',
    specs:[['Формат','.exe / .msi'],['Захват','WASAPI loopback'],['Статус','скоро']], cta:'Скачать для Windows' },
  { id:'mac', icon:'apple', name:'macOS', sub:'Grey Cardinal Daemon for macOS',
    specs:[['Формат','.dmg'],['Захват','ScreenCaptureKit'],['Поддержка','Apple Silicon / Intel']], cta:'Скачать для macOS' },
  { id:'linux', icon:'linux', name:'Linux', sub:'Grey Cardinal Daemon for Linux',
    specs:[['Формат','AppImage / deb'],['Захват','PipeWire / Pulse'],['Статус','скоро']], cta:'Скачать для Linux' },
];

const INSTALL = {
  win: ['Скачайте installer.', 'Запустите Grey Cardinal Daemon.', 'Разрешите доступ к системному звуку.', 'Введите workspace token.', 'Откройте встречу — агент начнёт слушать.'],
...
const PlatformDownloadCard = ({ p }) => (
  <div className="gc-plat">
    ...
    <button className="gc-btn gc-btn--secondary gc-btn--block" aria-disabled="true"
            title="Сборки появятся позже" style={{ opacity:.6, cursor:'not-allowed' }}>
      <Icon name="download" size={16}/>{p.cta}
    </button>
  </div>
);
      ...
      <section className="gc-section gc-section--tight">
        <div className="gc-wrap gc-reveal">
          ...
          <div className="gc-plat-grid">
            {PLATFORMS.map(p => <PlatformDownloadCard p={p} key={p.id}/>)}
          </div>
        </div>
      </section>
      ...
      <section className="gc-section gc-section--tight">
        <div className="gc-wrap gc-reveal">
          ...
          <p className="gc-mute" style={{ marginTop: 24, fontSize: 14, maxWidth: 640 }}>
            Daemon не ведёт проект самостоятельно. Он только превращает звук встречи в поток событий.
            Вся логика задач, подтверждений, доски и напоминаний живёт в brain-api.
          </p>
        </div>
      </section>
      ...
      <section className="gc-section gc-section--tight">
        <div className="gc-wrap gc-reveal">
          ...
          <table className="gc-status-table">
            ...
            <tbody>
              <tr><td>Windows</td><td><span className="gca-badge gca-badge--med">planned</span></td><td className="mono">.exe / .msi</td><td className="mono">WASAPI loopback</td></tr>
              <tr><td>macOS</td><td><span className="gca-badge gca-badge--med">planned</span></td><td className="mono">.dmg</td><td className="mono">ScreenCaptureKit / virtual device</td></tr>
              <tr><td>Linux</td><td><span className="gca-badge gca-badge--med">planned</span></td><td className="mono">AppImage</td><td className="mono">PipeWire / PulseAudio monitor</td></tr>
            </tbody>
          </table>
        </div>
      </section>
