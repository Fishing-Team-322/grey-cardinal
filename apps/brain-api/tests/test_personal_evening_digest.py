...
async def test_digest_command_in_private_returns_personal(make_uow, seed_chat, telegram, config):
    user = await _user(make_uow, tg_id=555)
    await _task(make_uow, seed_chat, user_id=user.id, deadline=NOW + timedelta(days=1))
    async with make_uow() as uow:
        actions = await SendPersonalEveningDigests(uow, telegram, config).as_actions_for_user(
            555, 555
        )
