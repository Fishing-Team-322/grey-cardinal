...
export default async function aiInboxView(root, params) {
  ...
  async function load() {
    ...
    content.innerHTML = visible.length ? visible.map(renderItem).join("") : '<div class="note">Очередь пуста.</div>';
...
function renderItem(item) {
  ...
  return `<article class="inbox-item">
    ...
    ${item.status === "pending" ? `<div class="inbox-actions"><button class="btn btn-sm btn-primary" data-approve="${item.id}">Создать</button><button class="btn btn-sm btn-ghost" data-reject="${item.id}">Отклонить</button></div>` : ""}
  </article>`;
}
